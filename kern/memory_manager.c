...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* page_table_temp = NULL;
	uint32* pageTableStartAddress = kmalloc(PAGE_SIZE);
	uint32 ret=kheap_physical_address((uint32)pageTableStartAddress);
	uint32 pageTableStartAddressPhys=CONSTRUCT_ENTRY(ret,PERM_WRITEABLE|PERM_PRESENT|PERM_USER);
	int i;
	for(i = 0; i < 1024; i++){
		pageTableStartAddress[i] = 0;
	}
	tlbflush();
	ptr_page_directory[PDX(virtual_address)] = pageTableStartAddressPhys;
	return (void*)pageTableStartAddress;
}
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 i;
	for(i = virtual_address; i < virtual_address + size; i += PAGE_SIZE){
		int createResult = pf_add_empty_env_page(e, i, 0);
		if(createResult == E_NO_PAGE_FILE_SPACE){
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(i=virtual_address;i<(virtual_address+size);i+=PAGE_SIZE){
		...
		pf_remove_env_page(e,i);//Remove an existing environment page at the given virtual address from the page file.
		//2. Free ONLY pages that are resident in the working set from the memory
		env_page_ws_invalidate(e, i);//remove working set entry
		unmap_frame( e->env_page_directory,(void*)i);// unmap working set memory
	}

	uint32 VirtAddr=ROUNDDOWN(virtual_address,PAGE_SIZE);
	for(i = VirtAddr; i < virtual_address + size; i+=(PAGE_SIZE)){//loop overall pageTables
		uint32* ptr = NULL;
