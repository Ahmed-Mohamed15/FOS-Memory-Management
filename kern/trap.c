...
void placement(uint32 fault_va){
	struct Frame_Info* tempFrameInfo;
			// allocate & map
			allocate_frame(&tempFrameInfo);
			map_frame(curenv->env_page_directory, tempFrameInfo, (void*)fault_va, PERM_WRITEABLE|PERM_USER|PERM_PRESENT);
			// read faulted page from page file to main memory
			if(E_PAGE_NOT_EXIST_IN_PF == pf_read_env_page(curenv, (void*)fault_va)){ // check if reading is successful
				// 	if page doesn't exist, check if it's a stack page
				if(fault_va < USTACKTOP && fault_va >=USTACKBOTTOM){ // ask if it's < or <=, ask if it has to be rounded
					// if it's a stack page, add new empty page with the faulted address to the page file
					int addPageResult = pf_add_empty_env_page(curenv, fault_va, 0);
					if(addPageResult == E_NO_PAGE_FILE_SPACE){
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if(curenv->page_WS_max_size > env_page_ws_get_size(curenv)){//placement
		placement(fault_va);
	}
		...
		if(isPageReplacmentAlgorithmCLOCK()){
			uint32 indx=curenv->page_last_WS_index;
			while(1){

				uint32 vAddr = env_page_ws_get_virtual_address(curenv, indx);
				if((pt_get_page_permissions(curenv,vAddr)&PERM_USED)==PERM_USED){
					pt_set_page_permissions(curenv,vAddr,0,PERM_USED);
					indx++;
					if(indx==curenv->page_WS_max_size)
						indx=0;
				}
				else{
					curenv->page_last_WS_index=indx+1;
					if(curenv->page_last_WS_index==curenv->page_WS_max_size)
						curenv->page_last_WS_index=0;
					pt_set_page_permissions(curenv,vAddr,1,PERM_USED);
					victimIndex=indx;
					break;
				}
			}
			...
			uint32 leastRecentTime = -1,TimeTemp;
			int i;
			for(i = 0; i < curenv->page_WS_max_size; i++){
				TimeTemp= env_page_ws_get_time_stamp(curenv, i);
				if(leastRecentTime == -1 ||TimeTemp < leastRecentTime){
					leastRecentTime = TimeTemp;
					victimIndex = i;
				}
			}
		}
			...
			FifoIndex++;
			if(FifoIndex== curenv->page_WS_max_size){
				FifoIndex=0;
			}
		}
		uint32 victimVA = env_page_ws_get_virtual_address(curenv, victimIndex);
		struct Frame_Info* frameInfoTemp;
		if((pt_get_page_permissions(curenv, victimVA) & PERM_MODIFIED) == PERM_MODIFIED){
			uint32* ptrPageTable = NULL;
			get_page_table(curenv->env_page_directory, (void*)victimVA, &ptrPageTable);
			frameInfoTemp = get_frame_info(curenv->env_page_directory, (void*)victimVA, &ptrPageTable);
			pf_update_env_page(curenv, (void*)victimVA, frameInfoTemp);
		}
		unmap_frame(curenv->env_page_directory, (void*)victimVA);
		env_page_ws_clear_entry(curenv, victimIndex);
		placement(fault_va);
	}
